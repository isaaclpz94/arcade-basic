...
function my_meta(){
	add_meta_box(
	'my_custom_meta',
	'Otra información (comprobar que este todo relleno)',
	'my_meta_box_mes',
	'post',
	'advanced',
	'core'
...
function my_meta_box_mes(){
	...
	<input type="number" name="precio" id="precio" value="<?php echo esc_attr(get_post_meta(get_the_ID(),'precio', true)); ?>"> <br/><br/>
	<strong> E-mail </strong><br/>
	<input type="text" name="email" id="email" value="<?php echo esc_attr(get_post_meta(get_the_ID(),'email', true)); ?>"> <br/><br/>
	<?php
}

function save_extra_profile_fields($post_id){
    update_post_meta($post_id,'precio',$_POST['precio']);
    update_post_meta($post_id,'email',$_POST['email']);
}
add_action('save_post','save_extra_profile_fields'); //La función se ejecuta cuando se guarda o edita un post

function save_extra_profile_fields_manual($post_id, $precio, $email){ //Se le pasan los valores de los campos meta manualmente
...
function demo_newpage_search() {
	$args = array();
	$args['wp_query'] = array( 'post_type' => array('post', 'gs_field', 'gs_param'), 
	                           'orderby' => 'title', 
	                           'order' => 'ASC' );
	                           
	// Here is where we specify the page where results will be shown
	$args['form'] = array( 'action' => get_bloginfo('url') . '/pagina-ejemplo' );
	//
	...
	$args['fields'][] = array( 'type' => 'post_type', 
	                           ...
	                           'values' => array('page' => 'Pages', 'gs_field' => 'Fields', 'gs_param' => 'Parameters') ,
	                           'default_all' => true
	                           );
	$args['fields'][] = array( 'type' => 'orderby', 
	                           'format' => 'select', 
	                           'label' => 'Order by', 
	                           'values' => array('title' => 'Title', 'date' => 'Date Added') );
	...
	$args['fields'][] = array( 'type' => 'taxonomy',
								'taxonomy' => 'category',
								'exclude' => array('peligros','alfacar'),
								'operator'=> 'AND',
								'format' => 'select');
	$args['fields'][] = array( 'type' => 'submit',
	                           'class' => 'button',
...
add_action('init','demo_newpage_search');
...
function bavotasan_add_js() {
	...
	wp_enqueue_script( 'tiempo', BAVOTASAN_THEME_URL .'/library/js/tiempo.js', array(), '', true );
